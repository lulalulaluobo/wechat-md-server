...
EOF
  ...
  # 备份密钥到 data 目录下，以防 docker-compose.yml 丢失或被覆盖
  cat >"${DATA_DIR}/.env_backup" <<EOF
WECHAT_MD_APP_MASTER_KEY="${master_key}"
WECHAT_MD_ADMIN_PASSWORD="${admin_password}"
EOF
}
...
prepare_directories() {
  ...
  # 容器以 root (user: 0:0) 运行，宿主机目录无需特殊 chown
...
update_app() {
  ...
  # 如果正则没拿到，尝试从备份文件恢复
  if [[ -z "${master_key}" ]] && [[ -f "${DATA_DIR}/.env_backup" ]]; then
    master_key=$(grep "^WECHAT_MD_APP_MASTER_KEY=" "${DATA_DIR}/.env_backup" | cut -d'"' -f2)
  fi
  if [[ -z "${admin_password}" ]] && [[ -f "${DATA_DIR}/.env_backup" ]]; then
    admin_password=$(grep "^WECHAT_MD_ADMIN_PASSWORD=" "${DATA_DIR}/.env_backup" | cut -d'"' -f2)
  fi
...
main_menu() {
  while true; do
    clear
    echo -e "${BOLD}wechat-md-server 部署菜单${NC}"
    echo "1. 安装 / 初始化部署"
    echo "2. 更新镜像并重启"
    echo "3. 查看服务状态"
    echo "4. 查看日志"
